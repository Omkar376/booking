...
#cur.execute('update drivers set driver_status = "Available"')
#conn.commit()
...
def driver_status():
    ...
    print(result)
    result = [('Driver Name', 'Status', 'Driver Address', 'Customer' ,'Customer Address')] + result
    print(result)
    conn.commit()
    cur.close()    
    return render_template('driver_status.html', driverDetials = result)
